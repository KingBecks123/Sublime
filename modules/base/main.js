...
function startingMessages(){
	if (game.lookAround < 1) {
		game.collectLimesAtBeginning += 1
	}

	if (game.collectLimesAtBeginning <= messages.length && game.lookAround == 0) {
		update("newInfo", messages[game.collectLimesAtBeginning - 1])
	}
}

function getLimes() {
	canFindNothing = (game.keenEyeSkillLevel != game.keenEyeSkillLevelMax);
...
function updateValuesBase () {
	if (game.sfxOn) {
		...
		sfxToggleButton.style.backgroundColor = myLime
	} else {
		update('sfxToggleButton', '♪ Off')
		sfxToggleButton.style.backgroundColor = '#898989'
	}

	if (game.showDonationButton) {
    ...
    const conditions = [
      { condition: game.lookAround > 1, elementId: 'sellYourLimesDiv' },
      { condition: game.lookAround > 2 && !(game.hideMaxedPurchases && game.juicers == game.juicersMax), elementId: 'buyAJuicerDiv' },
      { condition: game.lookAround > 2 && !(game.hideMaxedPurchases && game.baskets == game.basketsMax), elementId: 'buyABasketDiv' },
      { condition: game.tomes > 3, elementId: 'goldenBarDiv' },
      { condition: !game.pinUnlock, elementId: 'pinUnlockDiv' },
      { condition: game.pieBucket && game.pieFlourBucket, elementId: 'bucketThinSteelPlating' },
      { condition: game.maps == 0, elementId: 'buyAMapDiv1' },
      { condition: game.maps == 1, elementId: 'buyAMapDiv2' },
      { condition: game.maps == 2, elementId: 'buyAMapDiv3' },
      { condition: game.maps == 3, elementId: 'buyAMapDiv4' },
      { condition: game.maps == 4, elementId: 'buyAMapDiv5' },
      { condition: game.bulkBuyUnlock, elementId: 'basketsBulkButton', display: 'inline' },

	//Map Unlocks
      { condition: game.maps > 0 || game.villageNumber > 1, elementId: 'marketMainButtonsDiv', display: 'inline' },
      { condition: game.maps > 0, elementId: 'marketStore', display: 'block' },

      { condition: game.maps > 1, elementId: 'hiringAreaButton', display: 'inline' },
      { condition: game.maps > 1 && !game.storageUnlock, elementId: 'storageUnlockDiv' },
      { condition: game.maps > 1 && game.storageUnlock && !(game.storageJuicersUnlock && game.storagePeelersUnlock), elementId: 'storageDiv' },
      { condition: game.maps > 1 && !game.bulkBuyUnlock, elementId: 'bulkBuyUnlockDiv' },
      { condition: game.maps > 1 && game.bulkBuyUnlock && !game.bulkBuyUnlock2, elementId: 'bulkBuyUnlock2Div' },

      { condition: game.maps > 2, elementId: 'travellingArea' },
      { condition: game.maps > 2 && !game.fasterTransport, elementId: 'fasterTransportDiv' },
      { condition: game.maps > 2, elementId: 'increaseJuicePrice' },
      { condition: game.maps > 2 || game.villageNumber > 1, elementId: 'travelButton', display: 'inline' },

      { condition: game.maps > 3 && !game.respectBillboard, elementId: 'respectBillboard'},
      { condition: game.maps > 3, elementId: 'tasksButton' },
      { condition: game.maps > 3 && !game.autoCurrencyConversionBuy, elementId: 'autoCurrencyConversion' },

      { condition: game.maps > 4 && game.basicAlphaToBetaBroker, elementId: 'basicAlphaToBetaBrokerRule' },
      { condition: game.maps > 4 && !game.basicAlphaToBetaBroker, elementId: 'basicAlphaToBetaBroker' },
      { condition: game.maps > 4, elementId: 'buyPie' },

      { condition: game.lookAround != 3, elementId: 'lookAroundButton', display: 'inline' },
      { condition: game.villageNumber > 1 || game.betterTraining || game.increaseJuicePricePermanance, elementId: 'megaCoinUpgradesButton' },
      { condition: !game.forestWell, elementId: 'buyAWell' },
      { condition: game.respectMilestone10000 && !game.rottenActualWisdom, elementId: 'rottenActualWisdom' },
      { condition: !game.storagePeelersUnlock, elementId: 'storagePeelersDiv' },
      { condition: !game.storageJuicersUnlock, elementId: 'storageJuicersDiv' },
      { condition: !game.changeResearchersBy10Unlock, elementId: 'changeResearchersBy10Unlock' },
      { condition: !game.endScreen, elementId: 'sublimeMain'},
      { condition: !game.endScreen, elementId: 'sublimeMain'},
      { condition: game.endScreen, elementId: 'endScreen'},
      { condition: game.lookAround, elementId: 'navigateButtons', display: 'visible'}
    ];
